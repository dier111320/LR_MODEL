...
def file_append_file(source_filename,dest_file_name):
    source_f = open(source_filename,'r')
    dest_f = open(dest_file_name,'a')
    if os.path.isfile(dest_file_name) and os.path.getsize(dest_file_name):
        dest_f.write('\n')
...
#
#生成date文件夹【trian_pos,train_neg,test_pos,test_neg】,
#并遍历train_list和test_list的时间，将feature_oneday下文件 feature_selection.pos，feature_selection.neg
#合并到生成的文件夹下
#Base_Dir_208 = 根目录
#date : 2016-01-12
#train_list :['2015-10-12','2015-10-13']
#test_list :['2015-10-12','2015-10-13']
#
#
def get_combine(baseDir, date, train_date_list, test_date_list, SUBDIR='data'):
    # 合并多天的特征子集文件（正负例），形成训练、测试集
    if baseDir.endswith("/"):
        baseDir = baseDir[:-1]

    train_path_names = ["train_pos","train_neg"]
    test_path_names = ["test_pos","test_neg"]
    append_file_names=("feature_selection.pos","feature_selection.neg")
    ...
    for deal_date in train_date_list:
        append_base_path = "%s/%s/feature_oneday/%s" % (baseDir,SUBDIR,deal_date)
        for file_name in append_file_names:
            for test_path in train_path_names:
                file_append_file("%s/%s" % (append_base_path,file_name), "%s/%s" % (file_paths[test_path],file_name))

    for deal_date in test_date_list:
        append_base_path = "%s/%s/feature_oneday/%s" % (baseDir,SUBDIR,deal_date)
        for file_name in append_file_names:
            for test_path in test_path_names:
                file_append_file("%s/%s" % (append_base_path,file_name), "%s/%s" % (file_paths[test_path],file_name))


def run(Base_Dir_208, Base_Dir_share, date, conf):
    try:
    ...
    # 获取训练、测试日期
    # get_train_test_date('2015-11-01',1,1)
